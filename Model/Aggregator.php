    ...
    public function rebuild(?string $fromDate = null, ?string $toDate = null): void
    {
        ...
        try {
            ...
            foreach (['daily', 'weekly', 'monthly'] as $periodType) {
                $periodStartExpr = $this->getPeriodStartExpr($periodType);
                $periodEndExpr = $this->getPeriodEndExpr($periodType);
            ...
            $connection->rollBack();
            $this->logger->critical('[Merlin_SalesInsights] Aggregate rebuild failed: ' . $e->getMessage(), ['exception' => $e]);
            throw $e;
        }
    }

    private function getPeriodStartExpr(string $periodType): string
    {
        return match ($periodType) {
            'daily' => 'DATE(so.created_at)',
            'weekly' => 'DATE_SUB(DATE(so.created_at), INTERVAL WEEKDAY(so.created_at) DAY)',
            'monthly' => 'DATE_FORMAT(so.created_at, "%Y-%m-01")',
            default => 'DATE(so.created_at)',
        };
    }

    private function getPeriodEndExpr(string $periodType): string
    {
        return match ($periodType) {
            'daily' => 'DATE(so.created_at)',
            'weekly' => 'DATE_ADD(DATE_SUB(DATE(so.created_at), INTERVAL WEEKDAY(so.created_at) DAY), INTERVAL 6 DAY)',
            'monthly' => 'LAST_DAY(so.created_at)',
            default => 'DATE(so.created_at)',
        };
    }
}
